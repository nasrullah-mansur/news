<?php

namespace Database\Seeders;

use App\Models\Admin\Translation;
use Illuminate\Database\Seeder;

class TranslationSeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {
        Translation::create([
            'pl_one' => 'Breaking News',
            'sl_one' => 'Breaking News SL',
            'pl_two' => 'News Category',
            'sl_two' => 'News Category SL',
            'pl_three' => 'Follow Us',
            'sl_three' => 'Follow Us SL',
            'pl_four' => 'Join 456k Followers',
            'sl_four' => 'Join 456k Followers SL',
            'pl_five' => 'All News',
            'sl_five' => 'All News SL',
            'pl_six' => 'Trending News',
            'sl_six' => 'Trending News SL',
            'pl_seven' => 'World News',
            'sl_seven' => 'World News SL',
            'pl_eight' => 'Sport News',
            'sl_eight' => 'Sport News SL',
            'pl_nine' => 'Entertainment News',
            'sl_nine' => 'Entertainment News SL',
            'pl_ten' => 'Video News',
            'sl_ten' => 'Video News SL',
            'pl_eleven' => 'Most Viewed Post',
            'sl_eleven' => 'Most Viewed Post SL',
            'pl_twelve' => 'Quick Links',
            'sl_twelve' => 'Quick Links SL',
            'pl_thirteen' => 'Categories',
            'sl_thirteen' => 'Categories SL',
            'pl_fourteen' => 'Read More',
            'sl_fourteen' => 'Read More SL',
            'pl_fifteen' => 'News Details',
            'sl_fifteen' => 'News Details SL',
            'pl_sixteen' => 'Comment Post',
            'sl_sixteen' => 'Comment Post SL',
            'pl_seventeen' => 'Leave A Reply',
            'sl_seventeen' => 'Leave A Reply SL',
            'pl_eighteen' => 'Write you text here . . .',
            'sl_eighteen' => 'Write you text here . . . SL',
            'pl_nineteen' => 'Your Name',
            'sl_nineteen' => 'Your Name SL',
            'pl_twenty' => 'Email Address',
            'sl_twenty' => 'Email Address SL',
            'pl_twenty_one' => 'Phone No',
            'sl_twenty_one' => 'Phone No SL',
            'pl_twenty_two' => 'Post Comment',
            'sl_twenty_two' => 'Post Comment SL',
            'pl_twenty_three' => 'Related News',
            'sl_twenty_three' => 'Related News SL',
            'pl_twenty_four' => 'News Categories',
            'sl_twenty_four' => 'News Categories SL',
            'pl_twenty_five' => 'Popular News',
            'sl_twenty_five' => 'Popular News SL',
            'pl_twenty_six' => 'Stay Connected',
            'sl_twenty_six' => 'Stay Connected SL',
            'pl_twenty_seven' => 'Recent News',
            'sl_twenty_seven' => 'Recent News SL',
            'pl_twenty_eight' => 'Gallery Images',
            'sl_twenty_eight' => 'Gallery Images SL',
            'pl_twenty_nine' => 'Privacy Policy',
            'sl_twenty_nine' => 'Privacy Policy SL',
            'pl_thirty' => 'Cookies',
            'sl_thirty' => 'Cookies SL',
            'pl_thirty_one' => 'Contact',
            'sl_thirty_one' => 'Contact SL',
            'pl_thirty_two' => 'Need To Talk?',
            'sl_thirty_two' => 'Need To Talk SL?',
            'pl_thirty_three' => 'Address',
            'sl_thirty_three' => 'Address SL',
            'pl_thirty_four' => 'Support Hour',
            'sl_thirty_four' => 'Support Hour SL',
            'pl_thirty_five' => 'Quick Contact',
            'sl_thirty_five' => 'Quick Contact SL',
            'pl_thirty_six' => 'Send Us Message',
            'sl_thirty_six' => 'Send Us Message SL',
            'pl_thirty_seven' => 'send massage',
            'sl_thirty_seven' => 'send massage SL',
            'pl_thirty_eight' => 'It is a long established fact that a reader distracted by the readable content of a page.',
            'sl_thirty_eight' => 'It is a long established fact that a reader distracted by the readable content of a page.',
            'pl_thirty_nine' => 'Request For Ad',
            'sl_thirty_nine' => 'Request For Ad',
            'pl_forty' => 'blank',
            'sl_forty' => 'blank',

        ]);
    }
}
